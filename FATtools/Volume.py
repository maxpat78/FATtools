...
def _preserve_attributes_between(attributes, base, dest, fpi, fpo):
    return
    if attributes: # bit mask: 1=preserve creation time, 2=last modification, 3=last access
        if attributes & 1:
            pass # utime does not support this
    ...
    for root, folders, files in base.walk():
        for file in files:
            ...
            while True:
                ...
                if not s: break
                fpo.write(s)
            _preserve_attributes_between(attributes, base, dest, fpi, fpo)
            fpo.close()
            fpi.close() # If closing is deferred to atexit, massive KeyError exceptions are generated by disk.py in cache_flush: investigate!
...
def copy_between(src_list, src_dir, dest_path, dest_dir, callback=None, attributes=None, chunk_size=1<<20):
    ...
    for it in src_list:
        ...
        while True:
            ...
            if not s: break
            fpo.write(s)
        _preserve_attributes_between(attributes, src_dir, dest_dir, fpi, fpo)
        fpo.close()
        fpi.close()
