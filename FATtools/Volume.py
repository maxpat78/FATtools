...
def _preserve_attributes_between(attributes, base, fpi, fpo):
    if attributes: # bit mask: 0=preserve creation time, 1=last modification, 2=last access
        # 5=zero last modification & access times (MS-DOS <7)
        if attributes & 1:
            if base.fat.exfat:
                fpo.Entry.dwCTime = fpi.Entry.dwCTime
                fpo.Entry.chmsCTime = fpo.Entry.chmsCTime
            else:
                fpo.Entry.wCDate = fpi.Entry.wCDate
                fpo.Entry.wCTime = fpi.Entry.wCTime
        if attributes & 2:
            if base.fat.exfat:
                fpo.Entry.dwMTime = fpi.Entry.dwMTime
            else:
                fpo.Entry.wMDate = fpi.Entry.wMDate
                fpo.Entry.wMTime = fpi.Entry.wMTime
        if attributes & 4:
            if base.fat.exfat:
                fpo.Entry.dwATime = fpi.Entry.dwATime
            else:
                fpo.Entry.wADate = fpi.Entry.wADate
...
def copy_tree_between(base, dest, callback=None, attributes=None, chunk_size=1<<20):
    """Copy recursively files and directories under virtual 'base' Dirtable into
    virtual 'dest' Dirtable, 'chunk_size' bytes at a time, calling callback function if provided
    and preserving date and times if desired."""
    for root, folders, files in base.walk():
        for file in files:
            src = os.path.join(root, file)
            dst = os.path.join(dest.path, src[len(base.path)+1:])
            if base.path == os.path.dirname(src):
                fpi = base.open(file)
            else:
                fpi = base.opendir(os.path.dirname(src)[len(base.path)+1:]).open(file)
            assert fpi.IsValid != False
            relative_dir = os.path.dirname(dst[len(dest.path):])
            # Split subdirs in target path
            subdirs = []
            while 1:
                pro, epi = os.path.split(relative_dir)
                ...
                subdirs += [epi]
            subdirs.reverse()
            # Recursively open path to dest_path, creating directories if necessary
            target = dest
            for subdir in subdirs:
                target = target.mkdir(subdir)
            fpo = target.create(os.path.basename(dst), (fpi.Entry.dwFileSize+target.boot.cluster-1)//target.boot.cluster)
            if callback: callback(dst) # strip base path
            while True:
                s = fpi.read(chunk_size)
                if not s: break
                fpo.write(s)
            _preserve_attributes_between(attributes, base, fpi, fpo)
            fpo.close()
            fpi.close() # If closing is deferred to atexit, massive KeyError exceptions are generated by disk.py in cache_flush: investigate!


def copy_between(src_list, src_dir, dest_path, dest_dir, callback=None, attributes=None, chunk_size=1<<20):
    """Given 'src_dir' and 'dest_dir' directory tables, copies files and directories in 'src_list'
    to 'dest_dir', 'chunk_size' bytes at a time, calling callback function if provided
    and preserving date and times if desired."""
    if dest_path in ('/','\\','.'):
        target = dest_dir
    else:
        relative_dir = dest_path
        # Split subdirs in target path
        subdirs = []
        while 1:
            pro, epi = os.path.split(relative_dir)
    ...
    for it in src_list:
        ...
        if '*' in it or '?' in it:
            subp = os.path.dirname(it)
            if subp:
                src_dir = src_dir.opendir(subp)
                if not src_dir:
                    raise FileNotFoundError("Source directory '%s' does not exist!"%subp)
            it = os.path.basename(it)
            if DEBUG&2: log("copy_between: searching for '%s' in '%s'", it, src_dir.path)
            for name in src_dir.listdir():
                if fnmatch.fnmatch(name, it):
                    src_list += [name]
            continue
        if DEBUG&2: log("copy_between: probing '%s' as file", it)
        fpi = src_dir.open(it)
        if not fpi.IsValid:
            # if existent but invalid, it is a dir
            if DEBUG&2: log("copy_between: probing '%s' as directory", it)
            fpi = src_dir.opendir(it)
            if not fpi:
                if DEBUG&2: log("copy_between: '%s' does not exist", it)
                if callback: callback('"%s" does not exist!'%it)
                continue
            try:
                prev_target = target
        ...
        while True:
            ...
            if not s: break
            fpo.write(s)
        _preserve_attributes_between(attributes, src_dir, fpi, fpo)
        fpo.close()
        fpi.close()
