...
class BAT(object):
    ...
    def __getitem__ (self, index):
        ...
        if index < 0:
            index += self.size
        if DEBUG&8: log("%s: requested to read GT[0x%X]", self.stream.name, index)
    ...
    def _isvalid(self, selftest=1):
        ...
        for i in range(self.size):
            ...
            if a*512 > last_block or a*512+raw_size > ssize:
                if DEBUG&4: log("%s: block %d offset (sector 0x%X) exceeds allocated file size", self, i, a)
            ...
            if (a*512-first_block)%raw_size:
                if DEBUG&4: log("%s: BAT[%d] offset (sector 0x%X) is not aligned", self, i, a)
...
class Extent(object):
    ...
    def read(self, size=-1):
        ...
        while size:
            ...
            if DEBUG&4: log("%s: reading Grain %d, offset 0x%X (vpos=0x%X, epos=0x%X)", self.name, self._pos//self.block, offset, self._pos, self.stream.tell())
            if leftbytes <= size:
                got=leftbytes
                size-=leftbytes
            else:
            ...
            if not block:
                if self.Parent:
                    ...
                    if DEBUG&4: log("%s: grain content is virtual (zeroed)", self.name)
                    buf+=bytearray(got)
                continue
...
class Image(object):
    ...
    def read(self, size=-1):
        ...
        while size:
            ...
            buf += subbuf
            if DEBUG&8: log("%s: read %d bytes from 0x%X (-0x%X)", self.name, len(subbuf), self._pos, extent['start'])
        return buf
...
def mk_dynamic(name, size, block=(64<<10), overwrite='no'):
    ...
    if DEBUG&4: log("making new Dynamic VMDK '%s' of %.02f MiB with block of %d bytes", name, float(size//(1<<20)), block)
...
def mk_diff(name, base, overwrite='no'):
    ...
    if DEBUG&4: log("making new Differencing VMDK '%s' of %.02f MiB", name, float(ima.size//(1<<20)))
